...
class UploadHandler
{
    ...
    public function __construct($options = null, $initialize = true, $error_messages = null, $uploaddir='public') {

        $this->options = array(
            'upload_path' => $uploaddir,
            'script_url' => $this->get_full_url().'/review/file',
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        if ($this->validate($uploaded_file, $file, $error, $index, $content_range)) {
            ...
            //여기에 먼가 더 추가를 해야 한다.
            $dao = new \system\dao\dao();
            $dao->open();
            $dao->queryOption=[
                'table' => 'Tbl_Image',
                'val' => [
                    'path' => $this->options['upload_path'],
                    'ImageName' => $file->name
                ]
            ];
            //echo $dao->getInsertQuery();
            $file->ImgIdx = $dao->ExecuteInsetLastId();
            $this->set_additional_file_properties($file);
        }
        return $file;
    }
